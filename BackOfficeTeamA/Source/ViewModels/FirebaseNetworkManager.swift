...
final class FirebaseNetworkManager: ObservableObject {
    @Published var infoManager: FirestoreCRUDProtocol?
    
    private let superCollectionName: String
    
    init(superCollectionName: CollectionType) {
        self.superCollectionName = superCollectionName.rawValue
        self.setupInfoManager(typeOf: superCollectionName)
    }
    
    func setupInfoManager(typeOf type: CollectionType) {
        switch type {
        case .store:
            infoManager = StoreNetworkManager(with: self.superCollectionName)
        default:
            print()
        }
    }
    ...
    //                let email = requestedData["email"] as? String ?? "NONE"
    //
    //                dump("\(id), \(lastLoginDate), \(createdDate), \(email)")
    //            }
    //        } catch {
