...
struct ReportView: View {
    ...
    @State var searchFor = ""
    @State private var reportData = TempReportModel.reportData.sorted(using: KeyPathComparator(\.reported))
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            VStack(alignment: .leading) {
                Table(reportData, selection: $selection, sortOrder: $sortOrder    ) {
            ...
            .onChange(of: sortOrder, perform: { newOrder in
                reportData.sort(using: newOrder)
            })
            //                .listStyle(.insetGrouped)
            .navigationDestination(for: TempReportModel.self) { reportData in
                ReportDetailView(reportModel: reportData)
            }
...
struct TempReportModel: Hashable, Identifiable {
    ...
    static var reportData: [TempReportModel] = [
